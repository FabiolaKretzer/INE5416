...
%gramatica para T2C
comando --> [figclone], [N], [X], [Y], {
	atom_number(N, A), atom_number(X, B), 
	atom_number(Y, C), figuraclone(A, B, C) }.
comando --> [figpf], [Id], [N], {
	atom_number(Id, A), atom_number(N, B), 
	figuraparafrente(A, B)}.
comando --> [figpt], [Id], [N], {
	atom_number(Id, A), atom_number(N, B), 
	figuraparatras(A, B)}.
comando --> [figgd], [Id], [A], {
	atom_number(Id, X), atom_number(A, Y), 
	figuragiradireita(X, Y)}.
comando --> [figge], [Id], [A], {
	atom_number(Id, X), atom_number(A, Y), 
	figuragiraesquerda(X, Y)}.
